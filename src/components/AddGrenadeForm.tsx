...
import { Map, GrenadeType, DifficultyLevel, TeamType, ThrowType } from '@/types/map';
...
const AddGrenadeForm: React.FC<AddGrenadeFormProps> = ({ map, isOpen, onClose, onSuccess }) => {
  ...
  const [coordinateMode, setCoordinateMode] = useState<'throw' | 'landing' | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (uploadedFiles.setup_image) {
        setup_image_url = await handleFileUpload(uploadedFiles.setup_image, 'images');
      }
      if (uploadedFiles.aim_image) {
        aim_image_url = await handleFileUpload(uploadedFiles.aim_image, 'images');
      }
      if (uploadedFiles.result_image) {
        result_image_url = await handleFileUpload(uploadedFiles.result_image, 'images');
      }
          ...
          <div className="w-1/2 p-6">
            <h3 className="text-lg font-bold text-white mb-4">
              {isSelectingCoordinates 
                ? `Кликните на карту для ${coordinateMode === 'throw' ? 'точки броска' : 'точки падения'}`
            ...
            <div className="relative bg-slate-700 rounded-lg overflow-hidden">
              <img
                ...
                className={`w-full aspect-video object-cover ${isSelectingCoordinates ? 'cursor-crosshair' : ''}`}
          ...
          <div className="w-1/2 p-6">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {throwTypeOptions.map((option) => (
                    <Badge
                      key={option.value}
                      variant={formData.throw_types.includes(option.value) ? "default" : "outline"}
                ...
                <div className="flex space-x-4">
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, media_type: e.target.value as 'video' | 'screenshots' })}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="file"
                      accept="video/*"
                      onChange={(e) => setUploadedFiles({ ...uploadedFiles, video: e.target.files?.[0] || null })}
                      className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => setUploadedFiles({ ...uploadedFiles, setup_image: e.target.files?.[0] || null })}
                      className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => setUploadedFiles({ ...uploadedFiles, aim_image: e.target.files?.[0] || null })}
                      className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => setUploadedFiles({ ...uploadedFiles, result_image: e.target.files?.[0] || null })}
                      className="w-full bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                    />
                  </div>
                </div>
              ...
              <div className="flex justify-end space-x-4 pt-4 border-t border-slate-700">
                ...
                <Button
                  type="submit"
                  disabled={loading || !formData.name || formData.throw_types.length === 0}
